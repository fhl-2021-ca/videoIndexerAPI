...
namespace VideoIndexerArm
{
    public static class VideoProgram
    {
        ...
        public static async Task<String> indexvideoAndGetInsights(string VideoUrl)
        {
            ...
            List<Sentiment> negativeInstances = sentiments.Find(x => x.SentimentType.Equals("Negative"));

            for(int i = 0; i < negativeInstances.Count; i++)
            {
                Instance instance = negativeInstances[i];
                Sentiment newSentiment = new Sentiment();
                newSentiment.SentimentType = instance.SentimentType;
                newSentiment.AverageScore = instance.AverageScore;
                newSentiment.Instances = instances;
                WorkingSet workingSet = new WorkingSet();
                workingSet.sentiment = ins
